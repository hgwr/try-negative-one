...
This is a sample to confirm that using `-1` in JPQL causes a JPQLException Syntax error when using EclipseLink.

## How to reproduce

Use the numeric literal `-1` in JPQL as follows.
...
The command `bash . /gradlew build` to build.

Run with command `java -jar build/libs/demo-0.0.1-SNAPSHOT.war`

Run the command `curl "http://127.0.0.1:8080/memo/category/negative-one"` from another terminal.
